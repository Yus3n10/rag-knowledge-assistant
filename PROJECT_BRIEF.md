...
- **Definitions blobs need per-term splitting.** `1910.21(b)` is a single 11,720-character paragraph with ~60 defined terms; `1910.140(b)` is 7,353 and `1910.134(b)` is 7,016. Median record is 127 characters. Note two different definition formats exist in the corpus: `Term means ...` (Subparts D and I) and `Term . Definition` (`1910.147(b)`), so a single pattern will not cover both. Generic chunking will either pull the whole blob (wasting context and diluting relevance for a query about one term) or cut it at an arbitrary window boundary mid-definition. These need special-case handling — split by defined term, each with its own identifier. Note the ingestion parser is already written and the corpus already built, so this is either a targeted re-ingest or a chunking-time split; decide which when Phase 2 starts.
